...
/*Esta es la vista del menu libros (opciones de los detalles del autor de un libro), aqui se muestra lo que el usuario vera en el software
@author Grupo 4
@Version 14/07/2021*/
@SuppressWarnings("serial")
public class MenuLibrosDetaAutorView extends MenuBaseView {
	 ...
	 * Create the panel.
	 */
	public MenuLibrosDetaAutorView() {
		this.setName("menuLibrosDetaAutor");
		...
		backBtn.getBtnImg().setIcon(new ImageIcon(MenuLibrosDetaAutorView.class.getResource("/icons/back_32px.png")));
		backBtn.setAllColors(defaultColors[0],defaultColors[1] , defaultColors[2]);
		
		this.regisBtn = new CustomMenuBtn(" Registrar");
		regisBtn.getBtnImg().setIcon(new ImageIcon(MenuLibrosDetaAutorView.class.getResource("/icons/add_user_male_32px.png")));
		regisBtn.setAllColors(defaultColors[0],defaultColors[1] , defaultColors[2]);
		
		this.modifBtn = new CustomMenuBtn(" Modificar");
		modifBtn.getBtnImg().setIcon(new ImageIcon(MenuLibrosDetaAutorView.class.getResource("/icons/registration_32px.png")));
		modifBtn.setAllColors(defaultColors[0],defaultColors[1] , defaultColors[2]);
		
		this.mostBtn = new CustomMenuBtn(" Mostrar todos");
		mostBtn.getBtnImg().setIcon(new ImageIcon(MenuLibrosDetaAutorView.class.getResource("/icons/eye_32px.png")));
		mostBtn.setAllColors(defaultColors[0],defaultColors[1] , defaultColors[2]);
		
		this.buscBtn = new CustomMenuBtn(" Buscar / Cód. Autor");
		buscBtn.getBtnImg().setIcon(new ImageIcon(MenuLibrosDetaAutorView.class.getResource("/icons/search_32px.png")));
		buscBtn.setAllColors(defaultColors[0],defaultColors[1] , defaultColors[2]);
		
		this.borBtn = new CustomMenuBtn(" Borrar");
		borBtn.getBtnImg().setIcon(new ImageIcon(MenuLibrosDetaAutorView.class.getResource("/icons/denied_32px.png")));
